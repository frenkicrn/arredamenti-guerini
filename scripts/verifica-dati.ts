...
const SLUG_ATTESI = ['cucine', 'soggiorno', 'tavoli-sedie', 'divani', 'camere', 'materassi', 'bagni'];
...
for (const c of CATEGORIE) {
  ...
  if (c.foto) assert.ok(existsSync(`src/assets/prodotti/${c.foto}.jpg`), `categoria ${c.slug}: foto ${c.foto}.jpg mancante`);
  for (const a of c.approfondimento) {
    if (a.foto) assert.ok(existsSync(`src/assets/prodotti/${a.foto}.jpg`), `categoria ${c.slug}: foto approfondimento ${a.foto}.jpg mancante`);
  }
...
assert.equal(SERVIZI.length, 3, 'devono esserci 3 servizi');
